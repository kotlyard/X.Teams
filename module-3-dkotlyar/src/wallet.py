...
def GetPrivateKeyFromFile(file, is_wif = 0):
	fd = open(file, "r")
	if fd == -1:
		print("Open error")
		return(-1)
	return(privateKeyToWif(fd.read()) if is_wif == 1 else fd.read())
...
def GetNewPublicAdress(priv_key):
	...
	main_key = "6f" + CreateMainnetKeyFromPublicKey(public_key)
...
def sign(priv, msg):
	...
	sig = sk.sign(bytes(msg, 'utf-8'), sigencode=ecdsa.util.sigencode_der_canonize)
